...
namespace EVMManagement.API.Controllers
{
    ...
    public class TransportsController : BaseController
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] TransportFilterDto filter)
        {
            ...
            {
                return BadRequest(ApiResponse<string>.CreateFail("Giá tr? PageNumber và PageSize ph?i l?n hon 0", null, 400));
            }
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            ...
            if (result == null)
            {
                return NotFound(ApiResponse<TransportResponseDto>.CreateFail("Không tìm th?y v?n chuy?n v?i mã yêu c?u", null, 404));
            }
        ...
        public async Task<IActionResult> Create([FromBody] TransportCreateDto dto)
        {
            if (!ModelState.IsValid)
            {
                ...
                return BadRequest(ApiResponse<TransportResponseDto>.CreateFail("D? li?u t?o v?n chuy?n không h?p l?", errors, 400));
            }
        ...
        public async Task<IActionResult> Update(Guid id, [FromBody] TransportUpdateDto dto)
        {
            ...
            {
                var errors = ModelState.Values.SelectMany(v => v.Errors).Select(e => e.ErrorMessage).ToList();
                return BadRequest(ApiResponse<TransportResponseDto>.CreateFail("D? li?u c?p nh?t v?n chuy?n không h?p l?", errors, 400));
            }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                ...
                if (!result)
                {
                    return NotFound(ApiResponse<string>.CreateFail("Không tìm th?y v?n chuy?n v?i mã yêu c?u", null, 404));
                }

                return Ok(ApiResponse<string>.CreateSuccess("Xóa v?n chuy?n thành công"));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ApiResponse<string>.CreateFail($"X?y ra l?i khi xóa v?n chuy?n: {ex.Message}", null, 500));
            }
        }
    }
}
